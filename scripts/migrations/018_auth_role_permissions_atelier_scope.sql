...
BEGIN
  IF EXISTS (
    ...
    UPDATE public.role_permission_atelier
    ...
    IF EXISTS (
      SELECT 1
      FROM pg_constraint
      WHERE conname = 'role_permission_atelier_role_permission_code_key'
    ) THEN
      ALTER TABLE public.role_permission_atelier
        DROP CONSTRAINT role_permission_atelier_role_permission_code_key;
    END IF;
