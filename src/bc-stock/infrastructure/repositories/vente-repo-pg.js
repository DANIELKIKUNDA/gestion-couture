...
export class VenteRepoPg {
  async getById(idVente) {
    const res = await pool.query(
      "SELECT id_vente, date_vente, total, total_prix_achat, benefice_total, statut, reference_caisse, motif_annulation FROM ventes WHERE id_vente = $1",
      [idVente]
    );
  ...
  async save(vente) {
    await pool.query(
      `INSERT INTO ventes (id_vente, date_vente, total, total_prix_achat, benefice_total, statut, reference_caisse, motif_annulation)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
       ON CONFLICT (id_vente)
       DO UPDATE SET date_vente=$2, total=$3, total_prix_achat=$4, benefice_total=$5, statut=$6, reference_caisse=$7, motif_annulation=$8`,
      [vente.idVente, vente.date, vente.total, vente.totalPrixAchat, vente.beneficeTotal, vente.statut, vente.referenceCaisse, vente.motifAnnulation || null]
    );
    ...
    for (const ligne of vente.lignesVente) {
      await pool.query(
        `INSERT INTO vente_lignes (
           id_ligne, id_vente, id_article, libelle_article, quantite, prix_unitaire, prix_achat_unitaire, benefice_unitaire, benefice_total
         )
