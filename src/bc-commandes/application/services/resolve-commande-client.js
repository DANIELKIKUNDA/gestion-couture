...
function createCommandeClientError(status, code, message, details = {}) {
...
function buildDuplicatePhoneError(client) {
  ...
  return createCommandeClientError(
    409,
    "CLIENT_DUPLICATE_PHONE",
    `Ce numero appartient deja a ${existingClient?.nomComplet || "un client existant"}. Utilisez ce client pour la commande.`,
    {
      existingClient
    }
  );
...
export async function resolveCommandeClientForCreation({
  idClient = "",
  nouveauClient = null,
  ...
  if (requestedClientId) {
    ...
    if (!existingClient) {
      throw createCommandeClientError(404, "CLIENT_NOT_FOUND", "Client introuvable.");
    }
    return {
  ...
  if (!normalizedClient?.nom || !normalizedClient?.prenom || !normalizedClient?.telephone) {
    throw createCommandeClientError(400, "CLIENT_DATA_INVALID", "Completez nom, prenom et telephone.");
  }
...
export function serializeCommandeClientConflict(err) {
  return {
