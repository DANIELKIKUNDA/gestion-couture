...
function subtitleFor(paiement) {
  return paiement?.motif || paiement?.referenceMetier || "Paiement de commande";
}
...
<template>
  <div class="commande-detail-payment-mobile-list">
    <p v-if="loading" class="helper">Chargement...</p>
    <MobileStateEmpty
      v-else-if="items.length === 0"
      title="Aucun paiement enregistre"
      description="Les paiements de cette commande apparaitront ici."
    />
    <template v-else>
      <MobileEntityCard
...
<style scoped>
.commande-detail-payment-mobile-list {
  display: grid;
  gap: 12px;
}
</style>
